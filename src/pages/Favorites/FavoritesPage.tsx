    ...
    <MainLayout>
      <Container className="py-4">
        <div className="d-flex align-items-center gap-3 mb-4">
          ...
          <h2 className="mb-0">Mis favoritos</h2>
        ...
        {items.length === 0 ? (
          <div className="text-center py-5 text-muted">
            ...
            <p className="fs-5">No tienes productos favoritos aún.</p>
                    ...
                    <Link to={slug} className="d-block position-relative">
                      ...
                      {(hasDiscount || hasGroupDiscount) && (
                        <Badge bg="danger" className="position-absolute top-0 start-0 m-2" style={{ fontSize: 10 }}>
                          {hasGroupDiscount ? `−${entity.discountPercent}%` : 'OFERTA'}
                        </Badge>
                      )}
                      {outOfStock && <div className="fav-card-outofstock">Sin stock</div>}
                    </Link>
                    <div className="p-2 d-flex flex-column gap-1">
                      ...
                      <div className="d-flex gap-1 mt-1">
                        ...
                        >
                          ...
                          {outOfStock ? 'Sin stock' : !item.variantId && entity.hasVariants ? 'Ver opciones' : 'Añadir'}
                        </Button>
                        <Button variant="outline-danger" size="sm" onClick={() => handleRemove(item)} title="Quitar de favoritos">
                          <FaTrash />
                        </Button>
                      </div>
                    </div>
